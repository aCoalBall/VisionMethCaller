...
import h5py
from remora.refine_signal_map_core import (extract_levels,)
...
def get_signal_event_from_fast5(file_name, reads_group="Raw/Reads", 
	...
	## rescale
	signal = np.array(scaling * (signal + offset), dtype=float)
	## normalization
	sshift, sscale = np.median(signal), float(robust.mad(signal))
	...
	'''
	Further normalize signals, refer to Remora
	'''
