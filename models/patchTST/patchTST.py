...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, 
        ...
        assert head_type in ['pretrain', 'prediction', 'regression', 'classification', 'basecaller'], 'head type should be either pretrain, prediction, basecaller or regression'
        # Backbone
        ...
        if head_type == "basecaller":
            self.head = BasecallerHead(self.n_vars, d_model, target_dim, head_dropout = head_dropout)
        elif head_type == "basecaller6mer":
            self.head = BasecallerHead_6mer(self.n_vars, d_model, target_dim, kmer = 6, head_dropout = head_dropout)
...
class BasecallerHead(nn.Module):
    def __init__(self, n_vars, d_model, n_classes, head_dropout) -> None:
        super().__init__()
        self.flatten = nn.Flatten(start_dim=1)
        self.dropout = nn.Dropout(head_dropout)
        self.linear = nn.Linear(n_vars*d_model, n_classes)
...
class BasecallerHead_6mer(nn.Module):
    def __init__(self, n_vars, d_model, n_classes, kmer, head_dropout) -> None:
        super().__init__()
        self.flatten = nn.Flatten(start_dim=1)
        self.dropout = nn.Dropout(head_dropout)
        self.linear = nn.Linear(n_vars*d_model, n_classes * kmer)
        self.unflatten = nn.Unflatten(1, (n_classes, kmer))
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = self.linear(x)
        y = self.unflatten(x)         # y: bs x n_classes x kmer
        return y
